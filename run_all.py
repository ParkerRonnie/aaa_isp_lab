...
"""一键跑完全部实验并生成报告。
    ...
    python run_all.py              # 完整体验（约 1-2 分钟）
    python run_all.py --fast       # 缩小图像与重复次数，快速验证

产出：out/report.md、out/report.html（含全部图表）、out/fig_*.png、out/results.json
"""
import argparse
import copy
import json
...
from src.sim.camera import (SimCamera, ev_limits, flicker_banding_metric)
from src.sim.sensor import SensorSim
from src.aaa.ae import AEController
from src.aaa.awb import AWBEstimator, ideal_gains, illuminant_error_deg
from src.color_science import rgb_to_cct_duv
from src.aaa import af as AF
from src.isp import modules as IM
from src.eval import metrics as MT
from src.eval import report as RP
...
def main():
    ...
    steps = [
        ("AE 测光", lambda: exp_ae_metering(scfg, icfg, size, ae_cfg)),
        ("AE 收敛", lambda: exp_ae_convergence(scfg, icfg, size, ae_cfg)),
        ("AE 标定曲线", lambda: exp_ae_curve(scfg, icfg, size, ae_cfg)),
        ("AE 曝光策略/抗闪烁", lambda: exp_ae_policy(scfg, icfg, size, ae_cfg)),
        ("AWB 对比", lambda: exp_awb(scfg, icfg, size, awb_cfg)),
        ("AWB 色温先验", lambda: exp_awb_constraint(scfg, icfg, size, awb_cfg)),
        ("CCM 与耦合", lambda: exp_ccm(scfg, icfg, size, awb_cfg)),
        ("AF 评价函数", lambda: exp_af_curves(scfg, icfg, af_cfg, size, repeats)),
        ("AF 搜索策略", lambda: exp_af_search(scfg, icfg, af_cfg, size)),
        ("3A 耦合", lambda: exp_coupling(scfg, icfg, ae_cfg, awb_cfg, af_cfg, size)),
    ]
    R = {}
    ...
    res = {
        "meta": {"time": time.strftime("%Y-%m-%d %H:%M:%S"),
                 ...
                 "n_captures": N_CAPTURES[0]},
        "isp": exp_isp(scfg, icfg, size),
        "ae_metering": R["AE 测光"],
        "ae_convergence": R["AE 收敛"],
        "ae_curve": R["AE 标定曲线"],
        "ae_policy": R["AE 曝光策略/抗闪烁"],
        "ae_flicker": exp_ae_flicker(scfg, icfg, size, ae_cfg),
        "awb": R["AWB 对比"],
    ...
    print(f"\n完成，用时 {time.time() - t0:.1f}s，成像 {N_CAPTURES[0]} 次")
    print(f"  {paths['md']}")
    print(f"  {paths['html']}")
    return paths
