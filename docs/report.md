...
numpy 1.26.4｜OpenCV 4.9.0｜gcc 8.1.0 | C++17 | release | Sep 19 2026 02:39:28

| 实现 | 中位数 µs | p10 | p90 | MAD |
|---|---|---|---|---|
| AWB 现状(Python) | 30285.6 | 29329.3 | 31520.2 | 490.6 |
| AWB 算法优化(Python) | 16860.2 | 16145.1 | 17721.1 | 463.9 |
| AWB C++ (double) | 1666.6 | 1642.4 | 1791.2 | 23.0 |
| AWB C++ (float32) | 1658.7 | 1634.3 | 1771.5 | 18.2 |
| AWB C++ (定点 Q16) | 3045.5 | 2915.6 | 3377.8 | 102.5 |
| AE 测光 现状(Python) | 841.3 | 824.9 | 975.9 | 9.6 |
| AE 测光 C++ (float32) | 354.7 | 343.2 | 383.1 | 10.5 |
| AE 全画面平均(Python) | 316.8 | 301.2 | 327.6 | 7.8 |
| AE 全画面平均 C++ | 342.0 | 337.2 | 398.9 | 4.3 |

ctypes 调用地板实测 **0.20 µs**，相对上面每个核都 < 5%，可以忽略。

**加速分解 —— 这张表才是这一节的重点：**

- 算法改进（Python→Python，只改算法）：**1.80×**
- 实现改进（算法不变，换 C++）：**10.16×**
- 总加速：**18.26×**

**「C++ 比 numpy 快 16 倍」是错的说法。** 看这一格就明白了：
在**没有任何算法差异**的模式上（全画面平均，两边都是一次求和），
换语言的收益是 **0.93×** —— 小于 1，也就是 **C 反而略慢**。
...
**估算的算力占比**：AWB 统计占 33ms 帧预算的 4.98%；
按像素数线性外推，1080p 约 19904 µs、4K 约 79618 µs。
