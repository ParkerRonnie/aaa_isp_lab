...
|  |  |
|---|---|
| 结论 | Tenengrad 评价函数与 MTF50 在整段镜头行程上**同步变化**，形状一致 |

AF 的评价函数（高频能量）本质上是在用**梯度统计量**去逼近物理清晰度 MTF。
这解释了它为什么会有"峰位抖动"和"假峰"：梯度统计量对噪声敏感，
而 MTF 是经过分箱平均的、噪声鲁棒得多的量。
高精度对焦系统之所以要专门的评价函数硬件统计模块，原因就在这里。
